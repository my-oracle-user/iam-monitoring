...
def bootstrap_environment_runtime(db_path, environment_id, initial_target_override=None):
    ...
    if not str(server.get("host") or "").strip():
        raise ValueError("OUD SSH host is required before bootstrap can run.")
    if not str(server.get("username") or "").strip():
        raise ValueError("OUD SSH Username is required before bootstrap can run.")
    ...
    if verify_result.get("exit_code") != 0:
        if runtime_target.get("sudoRequired"):
            raise ValueError(
                ...
                "Check that the SSH user can run sudo without a password after key-based login."
            )
        raise ValueError(
    ...
    bootstrap["runtimeKeyPath"] = runtime_key_path
